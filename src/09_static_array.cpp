...
int main() {
  ...
  // Пояснение: размер массива после его создания нельзя изменить.
  ...
  int arr_example_3[3] = {};  // вариант 3: с указанием размера, инициализировать все нулями

  // 2. Доступ к элементам статического массива.
  // Доступ к элементу массива через оператор [] (квадратные скобки).
  ...
  // "гарантируется", что элементы массива находятся последовательно в памяти, т.е. они все соседи
  // [0] [4] [8]  <-- соседние ячейки памяти (в скобках условный адрес в памяти)
  //  3   2   5   <-- значения, которые хранятся в ячейках памяти

  // указатель на ячейку номер 0: взятие адреса нулевого элемента
  int* arr_begin = arr_example;  // эквивалентно: ... = &arr_example[0]

  std::printf("arr_example[0] = %d\n", *arr_begin);
  std::printf("&arr_example[0] = %X\n", arr_begin);

  // передвигаем адрес, который хранится в указателе к следующей ячейке памяти (элементу)
  int* arr_next = arr_begin + 1;  // сдвиг значения адреса происходит на размер типа данных элемента массива

  // эквивалентная запись: ... = &arr_begin[1]
...
/*
 * Задания:
 * 1. Проблема определения размера массива.
 */